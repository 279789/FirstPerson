...
This project was primay writen for the exam of the Lecture : Object Orientierte Programmierung from Stephan Bökelmann at THGA.

The project contains a cpp program that demenstrates how a simple RaycasterEngine Operates.
...
How ever this gameengine is not truely a 3d Engine. It's more an 2D Game where Walls are drawn Vertically,
so that the View of the Player looks 3D Like.

Due to time constraints, this Version only renders walls, floor and seeling in the most simple way.
...
In order to run properbly, the `CmakeLists.txt` file needs the path of your CopperSpice directories.
On the current `CMakeLists.txt` are the paths of my directorys included. In order to run the program,
